...
function Piece(board,player,x,y,type,num,color){
	...
	this.x=x;						// the piece needs to know what its x location value is.
	this.y=y;						// the piece needs to know what its y location value is as well.
...
function Checker(parent) {
	...
	circ.setAttributeNS(null,"fill",this.parent.color);					// change the color according to player
	...
	var aniEle = document.createElementNS("http://www.w3.org/2000/svg","animateTransform");
	aniEle.setAttributeNS(null,'begin','indefinite');
	aniEle.setAttributeNS(null,'attributeName','transform');
	aniEle.setAttributeNS(null,'dur','1s');
	aniEle.setAttributeNS(null,'id','ani_'+this.parent.id);

	this.piece.appendChild(aniEle);
